...
async def _call_deepseek(client: AsyncOpenAI, model_name: str, system_prompt: str, user_content: str, json_mode: bool) -> str:
    """
    Handles calls specifically for DeepSeek. 
    Notes:
    - DeepSeek-V3 ('deepseek-chat') supports response_format={'type': 'json_object'}.
    - DeepSeek-R1 (Reasoner) does NOT support system prompts in the strict sense (it treats them as user info) or json_mode.
    - This implementation assumes 'deepseek-chat' (V3) standard behavior.
    """
    kwargs = { 
        "model": model_name, 
        "messages": [
    ...
    # Enable JSON mode only if explicitly requested and not using a reasoning model (like R1)
    # Adjust this condition if you use deepseek-reasoner
    if json_mode and "reasoner" not in model_name:
        kwargs["response_format"] = {"type": "json_object"}
    ...
    try:
        ...
        # Fallback: If JSON mode is rejected by the API (e.g. older model), try without it
        if json_mode and "400" in str(e):
            logger.warning(f"DeepSeek JSON mode failed, retrying without enforcement. Error: {e}")
